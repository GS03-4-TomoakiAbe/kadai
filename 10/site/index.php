<?php
include('../cms_sample/include/func.php'); //外部ファイル読み込み（関数群）
...
while( $result = $stmt->fetch(PDO::FETCH_ASSOC)){
  ...
  //管理FLGで表示を切り分けたりしてみましょう！！！（追加してください！）
    ...
    $view .= '<dt class="news-list--date">'.htmlEnc($result["indate"]).
    '</dt>'.'<dd class="news-list--note"><a href="news.php?id='.htmlEnc($result["id"]).'">'.
    htmlEnc($result["news_title"]).'</a></dd>';
	$count++;
	}
    if( $count >= 5){
	break;
...
<div class="wrapper">
    ...
    <?php
        include("../cms_sample/include/menu.php");
    ?>
